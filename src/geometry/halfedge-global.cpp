...
{
  // A2G1: triangulation
  auto IsVertexInTriangle = [&](VertexRef a, VertexRef b, VertexRef c, VertexRef p)
  {
    float d00 = dot(b->position - a->position, b->position - a->position);
    float d01 = dot(b->position - a->position, c->position - a->position);
    float d11 = dot(c->position - a->position, c->position - a->position);
    float d20 = dot(p->position - a->position, b->position - a->position);
    float d21 = dot(p->position - a->position, c->position - a->position);
  ...
  auto EarClipping = [&](HalfedgeRef h, HalfedgeRef hNext, HalfedgeRef hPrev)
  {
    EdgeRef eNew = emplace_edge(false);
    HalfedgeRef hNew = emplace_halfedge();
    interpolate_data({h}, hNew);

    HalfedgeRef tNew = emplace_halfedge();
    interpolate_data({h}, tNew);
    ...
    hNew->next = hPrev;
    hNew->edge = eNew;
    hNew->vertex = hNext->vertex;

    tNew->twin = hNew;
    tNew->next = hNext;
    tNew->face = hNext->face;
    tNew->edge = eNew;
    tNew->vertex = hPrev->vertex;

    fNew->halfedge = hNew;
    eNew->halfedge = hNew;

    HalfedgeRef hPrevPrev = hNext;
    while (hPrevPrev->next != hPrev)
    {
      hPrevPrev = hPrevPrev->next;
    }
    hPrevPrev->next = tNew;

    hNext->face->halfedge = hNext;
  };

  auto PerFaceOperation = [&](FaceRef f)
  {
    while (f->degree() > 3) {
      ...
      HalfedgeRef p = hNext->next;
      while (p != hPrev)
      {
        if (IsVertexInTriangle(h->vertex, hNext->vertex, hPrev->vertex, p->vertex))
          break;
        p = p->next;
      }

      if (p == hPrev)
        EarClipping(h, hNext, hPrev);

      h = hNext;
    }
  };
  FaceRef fEnd = faces.end();
  for (FaceRef f = faces.begin(); f != fEnd; f++)
  {
    if (!f->boundary)
      PerFaceOperation(f);
  }
...
void Halfedge_Mesh::catmark_subdivide()
{
  ...
  for (EdgeRef e = edges.begin(); e != edges.end(); e++)
  {
    ...
    if (!h->face->boundary && !t->face->boundary)
      edge_vertex_positions[e] = 0.25f * (h->vertex->position + t->vertex->position + face_vertex_positions.at(h->face) + face_vertex_positions.at(t->face));
    else
      edge_vertex_positions[e] = 0.5f * (h->vertex->position + t->vertex->position);
  }

  // Vertices
  for (VertexRef v = vertices.begin(); v != vertices.end(); v++)
  {
    bool onBoundray = false;
    ...
    {
      if (!h->face->boundary)
        Q += face_vertex_positions.at(h->face);
      else {
        onBoundray = true;
        VertexRef vNext = h->next->vertex;
        HalfedgeRef hPrev = h->next;
        while (hPrev->next->vertex != v)
          hPrev = hPrev->next;
        VertexRef vPrev = hPrev->vertex;
        vertex_positions[v] = (1.0f / 8.0f) * vNext->position + (1.0f / 8.0f) * vPrev->position + 0.75f * v->position;
        break;
      }
      R += h->edge->center();
      h = h->twin->next;
...
bool Halfedge_Mesh::loop_subdivide()
{
  ...
  {
    if (f->boundary)
      continue; // ignore boundary faces for this check
  ...
  [[maybe_unused]] std::unordered_map<VertexRef, Vec3> vertex_new_pos;
  for (VertexRef v = vertices.begin(); v != vertices.end(); v++)
  {
    bool onBoundray = false;
    float u;
    ...
    if (n == 3.0f)
      ...
      u = 3.0f / (8.0f * n);
    Vec3 sumOfNeighbor = Vec3(0.0f, 0.0f, 0.0f);
    HalfedgeRef h = v->halfedge;
    do {
      if (!h->face->boundary)
        sumOfNeighbor += h->twin->vertex->position;
      else
      {
        onBoundray = true;
        VertexRef vNext = h->next->vertex;
        ...
        while (hPrev->next->vertex != v)
          hPrev = hPrev->next;
        VertexRef vPrev = hPrev->vertex;
        vertex_new_pos[v] = (1.0f / 8.0f) * vNext->position + (1.0f / 8.0f) * vPrev->position + 0.75f * v->position;
        break;
      }
      h = h->twin->next;
    } while(h != v->halfedge);
    ...
    if (!onBoundray) {
      sumOfNeighbor *= u;
      vertex_new_pos[v] = (1.0f - n * u) * v->position + sumOfNeighbor;
    }
  }

  // Next, compute the subdivided vertex positions associated with edges, and
  // store them in edge_new_pos:
  [[maybe_unused]] std::unordered_map<EdgeRef, Vec3> edge_new_pos;
  for (EdgeRef e = edges.begin(); e != edges.end(); e++)
  {
    HalfedgeRef h = e->halfedge;
    HalfedgeRef t = h->twin;
    if (e->on_boundary())
      ...
      edge_new_pos[e] = 3.0f / 8.0f * (h->vertex->position + t->vertex->position) + 1.0f / 8.0f * (h->next->twin->vertex->position + t->next->twin->vertex->position);
  }
  ...
  [[maybe_unused]] std::vector<EdgeRef> new_edges;
  int numOfEdges = (int)edges.size();
  EdgeRef eCurrent = edges.begin();
  for (int i = 0; i < numOfEdges; i++)
  {
    EdgeRef eNext = eCurrent;
    eNext++;

    VertexRef v1 = eCurrent->halfedge->vertex;
    VertexRef v2 = eCurrent->halfedge->twin->vertex;
    VertexRef vNew = split_edge(eCurrent).value();

    HalfedgeRef h = vNew->halfedge;
    do{
      VertexRef vNewNeighbor = h->twin->vertex;
      if (vNewNeighbor != v1 && vNewNeighbor != v2)
  ...
  [[maybe_unused]] auto is_new = [&vertex_new_pos](VertexRef v) -> bool
  {
  ...
  for (VertexRef v = vertices.begin(); v != vertices.end(); v++)
  {
    if (vertex_new_pos.count(v))
      v->position = vertex_new_pos.at(v);
    else {
      HalfedgeRef h = v->halfedge;
      do
      {
        if (edge_new_pos.count(h->edge)) {
          v->position = edge_new_pos.at(h->edge);
          break;
        }
        h = h->twin->next;
      } while (h != v->halfedge);
...
void Halfedge_Mesh::set_corner_normals(float threshold)
{
  ...
  else
  {
    ...
    if (threshold <= 0.0f)
      cos_threshold = 2.0f; // make sure everything is sharp
    for (auto const &edge : edges)
    {
  ...
  for (auto const &v : vertices)
  {
    ...
    do
    {
      if (sharp_edges.count(&*begin->edge))
        break;
