...
namespace PT
{
	...
	Spectrum Pathtracer::sample_direct_lighting_task4(RNG &rng, const Shading_Info &hit)
	{
		...
		Materials::Scatter sctr = hit.bsdf.scatter(rng, hit.out_dir, hit.uv);
		Vec3 sample_direction = sctr.direction;
		// TODO: rotate that direction into world coordinates
		Vec3 world_direction = hit.object_to_world.rotate(sample_direction).unit();
		// TODO: construct a ray travelling in that direction
		//  NOTE: because we want emitted light only, can use depth = 0 for the ray
		Ray ray = Ray(hit.pos, world_direction, Vec2(EPS_F, std::numeric_limits<float>::infinity()), 0);
		// TODO: trace() the ray to get the emitted light (first part of the return value)
		Spectrum emitted;
		emitted = trace(rng, ray).first;
		...
		if (hit.bsdf.is_specular())
			radiance += emitted * sctr.attenuation;
		else
			radiance += emitted * sctr.attenuation / hit.bsdf.pdf(hit.out_dir, sample_direction);
	...
	Spectrum Pathtracer::sample_direct_lighting_task6(RNG &rng, const Shading_Info &hit)
	{
		...
		{
			sample_direction = sctr.direction;
		...
		radiance += emitted * hit.bsdf.evaluate(hit.out_dir, hit.world_to_object.rotate(world_direction), hit.uv) / average_pdf;
	...
	Spectrum Pathtracer::sample_indirect_lighting(RNG &rng, const Shading_Info &hit)
	{
		...
		Ray ray = Ray(hit.pos, world_direction, Vec2(EPS_F, std::numeric_limits<float>::infinity()), hit.depth - 1);
		// TODO: trace() the ray to get the reflected light (the second part of the return value)
		Spectrum reflected;
		reflected = trace(rng, ray).second;
		// TODO: weight properly depending on the probability of the sampled scattering direction and set radiance
		Spectrum radiance;
		if (hit.bsdf.is_specular())
			radiance = reflected * sctr.attenuation;
		else
			radiance = reflected * sctr.attenuation / hit.bsdf.pdf(hit.out_dir, sample_direction);
	...
	void Pathtracer::build_scene(Scene &scene_)
	{
		...
		{ // create scene instances
			...
			for (const auto &[name, mesh_inst] : scene_.instances.meshes)
			{
				...
				if (!mesh_inst->settings.visible)
					continue;
				if (mesh_inst->mesh.expired())
					continue;
			...
			for (const auto &[name, shape_inst] : scene_.instances.shapes)
			{
				...
				if (!shape_inst->settings.visible)
					continue;
				if (shape_inst->shape.expired())
					continue;
			...
			for (const auto &[name, part_inst] : scene_.instances.particles)
			{
				...
				if (!part_inst->settings.visible)
					continue;
				if (part_inst->mesh.expired())
					continue;
				if (part_inst->particles.expired())
					continue;
			...
			for (const auto &[name, light_inst] : scene_.instances.delta_lights)
			{
				...
				if (!light_inst->settings.visible)
					continue;
				if (light_inst->light.expired())
					continue;
