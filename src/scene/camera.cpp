...
std::pair<Ray, float> Camera::sample_ray(RNG &rng, uint32_t px, uint32_t py) {
  ...
  float radius = (vertical_fov / 2.0f) * (float)M_PI / 180.0f;
  float h = 2.0f * tan(radius);
  float w = h * aspect_ratio;

  Vec3 sensor_plane = Vec3(-w / 2.0f + w * sensor_pixel.x / (float)film.width,
                           -h / 2.0f + h * sensor_pixel.y / (float)film.height, -1.0f);

  // printf("Sensor pixel at %f, %f to plane %f, %f\n", sensor_pixel.x, sensor_pixel.y, sensor_plane.x, sensor_plane.y);
  //Build ray:
	Ray ray;
	ray.point = Vec3(); //ray should start at the origin
  ray.dir = sensor_plane.unit();  // TODO: compute from sensor plane position
